...
| O-018 | Tek proxy pod 120 saniye boyunca neden Ready olmadı? | İlk tanı `ob-network-proxy-readiness-001` opsiyonel condition `reason` alanında StrictMode ile incomplete kaldı; rollback/host geçti. Null-safe serializer, fixture ve yeni benzersiz diagnostic ID ile fault'suz tanı tekrarlanmalı | P2 no-fault proxy readiness diagnosis |
