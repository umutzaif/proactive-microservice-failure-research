...
| O-010 | D-030 hiçbir 15/20-user adayı seçmediğinde ikinci workload nasıl tasarlanmalı? | `P1-WORKLOAD-RESOURCE-BUDGET-001`, küçük adayın eski iki kapıyı birlikte çözmesinin beklenmediğini gösterdi. Karşılaştırılabilirliği koruyan teknik öneri: 15 user, değişmeyen limit/profiller ve yeni deneyler için prospektif `%5` (`10m`) nominal rezerv (`normal mean <=40m`). Bu kabul edilmiş karar değildir; açık kullanıcı onayı ve yeni preregistration gerekir | P3 ikinci workload öncesi |
