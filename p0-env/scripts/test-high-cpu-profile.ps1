...
$profilePath = Join-Path $repositoryRoot 'p0-env\config\faults\cpu-recommendation-high-v1.json'
$testRoot = Join-Path $repositoryRoot 'p0-env\state\tests\high-profile'
New-Item -ItemType Directory -Path $testRoot -Force | Out-Null

$positive = @(& powershell -NoProfile -ExecutionPolicy Bypass `
    -File (Join-Path $PSScriptRoot 'invoke-cpu-stress.ps1') `
    -ProfilePath $profilePath -RunId 'ob-cpu-high-test-001' `
...
if ($LASTEXITCODE -ne 0 -or ($positive -join "`n") -notmatch 'cpu_stress_injection=not-executed') {
    throw "High profile positive fixture failed: $($positive -join ' | ')"
}
...
$invalidProfile.physical_effect_verification.minimum_steady_minus_baseline_mean_millicores = 74
...
try {
    $negative = @(& powershell -NoProfile -ExecutionPolicy Bypass `
        -File (Join-Path $PSScriptRoot 'invoke-cpu-stress.ps1') `
        -ProfilePath $invalidProfilePath -RunId 'ob-cpu-high-test-002' `
...
if ($LASTEXITCODE -eq 0 -or ($negative -join "`n") -notmatch 'does not match its preregistered CPU-stress contract') {
    throw 'High profile verifier accepted a 74 mCPU minimum-increase gate.'
}

Write-Output 'high_cpu_profile_positive_whatif_fixture=passed'
Write-Output 'high_cpu_profile_minimum_increase_negative_fixture=passed'
