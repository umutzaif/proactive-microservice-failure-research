...
foreach($required in @('ob-network-resource-compat-005','run-manifest.json','telemetry_run_id','workload_profile_id','network-delay-resource-compatibility','explicit_execution_approval_required','working_tree_not_clean','immutable_output_exists','Observe 120','Observe 180','-RequireStable','500m','100m','proxy_clean_before_failed','proxy_clean_after_failed','New-HostEventRecordIdBoundary','Measure-HostEventsAfterRecordIdBoundary','Rollback','scientific_fault_started=$false','Invoke-NativeJsonCommand','kubectl-stderr.log','[string[]]$KubectlArguments','-ArgumentList $KubectlArguments')){if(-not$s.Contains($required)){throw "resource_compat_runner_contract_missing:$required"}}
