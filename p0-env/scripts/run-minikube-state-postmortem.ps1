...
function CaptureNative([string]$Name,[string]$FilePath,[string[]]$Arguments){$capture=Invoke-NativeCommandCapture -FilePath $FilePath -ArgumentList $Arguments;WriteJson(Join-Path $root $Name)([ordered]@{captured_utc=NowUtc;exit_code=$capture.exit_code;stdout=$capture.stdout;stderr=$capture.stderr})}
...
$dockerInfo=Invoke-NativeCommandCapture -FilePath $dockerExe -ArgumentList @('info','--format','{{.ServerVersion}}');$dockerVersion=$dockerInfo.stdout.Trim();if($dockerInfo.exit_code-ne0-or-not$dockerVersion){throw 'docker_engine_not_ready'}
$inspectCapture=Invoke-NativeCommandCapture -FilePath $dockerExe -ArgumentList @('inspect',$Profile);if($inspectCapture.exit_code-ne0-or-not$inspectCapture.stdout.Trim()){throw 'exact_profile_container_missing'}
$inspect=$inspectCapture.stdout|ConvertFrom-Json;if(@($inspect).Count-ne1){throw 'exact_profile_container_ambiguous'}
$containerState=[string]$inspect[0].State.Status;if($containerState-eq'running'){throw 'profile_container_not_stopped'}
New-Item -ItemType Directory -Path $root|Out-Null
$failure=$null
try{
 WriteJson(Join-Path $root 'diagnostic-manifest.json')([ordered]@{schema_version=1;gate_id='P2-MINIKUBE-STATE-POSTMORTEM-001';diagnostic_id=$DiagnosticId;code_revision=(& git -C $repo rev-parse HEAD).Trim();profile=$Profile;external_minikube_home=$externalMinikubeHome;resolved_minikube_home=$resolvedMinikubeHome;expected_minikube_home=$expectedMinikubeHome;profile_root=$profileRoot;docker_version=$dockerVersion;container_state=$containerState;profile_deleted=$false;container_started=$false;cluster_started=$false;application_manifest_applied=$false;workload_started=$false;toxic_created=$false;scientific_fault_started=$false;scientific_window_started=$false;dataset_inclusion=$false;headroom_decision_inclusion=$false})
 WriteJson(Join-Path $root 'container-inspect.json')$inspect[0]
 CaptureNative 'volume-inspect.json' $dockerExe @('volume','inspect',$Profile)
 CaptureNative 'docker-logs.json' $dockerExe @('logs','--timestamps',$Profile)
 CaptureNative 'minikube-last-start-logs.json' $minikubeExe @('logs','--profile',$Profile,'--last-start-only')
