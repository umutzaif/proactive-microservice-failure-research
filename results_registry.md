...
| P1-CPU-001 / ob-cpu-low-003 | 2026-08-04 | planned | Düzeltilmiş fiziksel-etki coverage kapısıyla düşük CPU-stress kalibrasyonunu toplamak | `cpu-recommendation-low-v2`; `p1-cpu-001-slo-v1`; seed 1 | Koşullar v1 ile aynı; her 300 sn baseline/steady fazında >=48 gerçek interval ve CPU artışı >=25m | Bekleniyor; run henüz başlatılmadı | `p0-env/artifacts/P1-CPU-LOW-PREREG-001/attempt-003.md` | `ob-cpu-low-002` invalid kalır. Temiz committed revision, bağımsız testler, PR merge ve etkin run-ID kapısı geçmeden başlatılmaz |
