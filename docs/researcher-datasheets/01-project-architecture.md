...
`active profile -> six fresh 500m normals -> headroom -> three delays/one workload selection ->
health-path isolation -> nine valid runs -> run-count stop gate -> finding or negative close`.
...
### D-111 normal convergence and failure capture

Normal execution becomes `proxy rollout -> bounded 120/5 full-selector convergence ->
unchanged 120/5 stability -> warm-up/baseline`. `wait-normal-proxy-convergence.ps1` uses
the existing readiness and optional-property helpers; it consumes a PodList and outputs
`proxy-pod-convergence.json` including raw observations on success, timeout or query error.
The existing stability verifier emits `<EvidencePath>.failure.json` on failure and retains
its original error; existing evidence paths cannot be overwritten. The new
`normal-failure-closure.ps1` captures host/network/profile/container state independently
after best-effort rollback/stop and writes `failure-closure.json`; errors remain explicit.

These helpers live with the runner to keep execution/evidence ownership together; they
are maintenance code, not scientific result files. Inputs are the exact profile, prior
host/network boundaries and run artifact path; dependencies are existing Windows,
Minikube/Docker and PowerShell utilities. Main risks are confusing rollout with complete
pod removal, accepting late readiness, hiding terminating pods and treating failed capture
as zero events. Fixture tests run with no cluster and challenge those boundaries. The new
test script is maintained alongside the helpers. This changes no service topology or
scientific window. `005` artifacts remain sealed and are never regenerated by these helpers.
### D-112 narrowed screen and scope boundary

The active path is `three remaining Ethernet normals -> six-run headroom -> three-delay,
one-workload selection -> health-path isolation -> nine valid runs`. Selection is sealed before
fault results. Invalid attempts retain evidence but do not advance the counter. Each run records
