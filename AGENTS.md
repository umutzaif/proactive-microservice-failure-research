...
The internship mentor's 2026-08-21 and 2026-09-09 reviews are binding for every future experiment task and Codex conversation in this repository. These gates apply prospectively; they do not rewrite or relabel historical evidence. The 2026-09-09 review supersedes only the former 2026-09-15 calendar gate and the full `5 x 2 x 3` screening requirement; all other validity gates remain in force.
...
- The candidate network-delay levels remain `25/50/100/250/500 ms`, but the first screening phase is narrowed prospectively after the six new 500m normal baselines and headroom analysis. The headroom record must select exactly three delay levels and one workload before any screening fault is run. Each selected level receives three independent valid repeats, for nine valid screening runs total. Invalid attempts do not count and do not change the selected cells.
...
- Preparation gate: by `2026-09-19`, the study must have six valid new 500m normal baselines, the sealed quantitative headroom analysis, and proof that the injected delay does not affect readiness/liveness/health paths. If this gate is missed, do not force or silently extend the scientific design; stop fault preparation and bring an alternative execution environment to the mentor for decision.
- Run-count stop gate: after the first nine valid narrowed-screening runs are complete, continue network delay only if at least one preregistered workload-delay cell produces frozen-SLO manifestation with at least 15 seconds positive lead time in at least 2 of its 3 valid repeats. Otherwise stop network delay as an early-prediction candidate and report the negative result. A different fault class requires an explicit research decision and separate preregistration.
- On the current host, long Wi-Fi runtime remains blocked by D-109 until vendor-supported remediation and a new clean-boot qualification. Ethernet is the active execution path. Do not repeat Wi-Fi qualification merely to unblock the ladder.
  D-115 prospectively permits only preregistered 10u-007 to use USB tethering through
...
Before material experiment planning or execution, run `p0-env/scripts/verify-mentor-feedback-policy.ps1`. A failed policy check is a stop condition; it cannot be waived by an agent or by an earlier runtime approval.
